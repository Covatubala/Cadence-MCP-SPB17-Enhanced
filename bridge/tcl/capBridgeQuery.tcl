...
proc ::capBridge::captureOn { pList } {
    ...
    if { !$capturing } {
        proc ::pcbWorkflows::puts { args } {
            ...
            # A few workflows deliberately log strings beginning with "\n".
            # The Communication Server protocol is one physical line per
            # response, so storing an embedded newline here desynchronizes the
            # persistent socket and makes the *next* workflow receive the tail
            # of the previous report. Split it into separate list elements.
            set msg [string map [list "\r\n" "\n" "\r" "\n"] [lindex $args end]]
            foreach line [split $msg "\n"] {
                lappend ::capBridge::logBuf $line
            }
...
# not a NULL check.
#############################################################################
proc ::capBridge::_findOpenLib { requested pStatus } {
    set found NULL
    set s $::DboSession_s_pDboSession
    DboSession -this $s
    set want [string tolower [string map [list "\\" "/"] $requested]]
    set it [$s NewLibsIter $pStatus]
    set lib [$it NextLib $pStatus]
    while { $lib ne "NULL" && $found eq "NULL" } {
        set c [DboTclHelper_sMakeCString]
        $lib GetName $c
        set actual [DboTclHelper_sGetConstCharPtr $c]
        if { [string tolower [string map [list "\\" "/"] $actual]] eq $want } {
            set found $lib
        }
        set lib [$it NextLib $pStatus]
    }
...
# openLibs -- enumerate the exact library names registered in this session.
# Call this before libPackages/pkgInfo rather than guessing a path: GetLib on
# a wrong UNC spelling has crashed Capture 17.4 in live testing.
proc ::capBridge::openLibs { pList } {
    set rows [list]
    set rc [catch {
        set s $::DboSession_s_pDboSession
        DboSession -this $s
        set st [DboState]
        set it [$s NewLibsIter $st]
        set lib [$it NextLib $st]
        while { $lib ne "NULL" } {
            set c [DboTclHelper_sMakeCString]
            $lib GetName $c
            lappend rows [DboTclHelper_sGetConstCharPtr $c]
            set lib [$it NextLib $st]
        }
        catch { $it -delete }
    } failed]
    if { $rc != 0 } { return [::capBridge::_err "openLibs: $failed"] }
    return [concat [list OK] $rows]
}
...
proc ::capBridge::libPackages { pList } {
    ...
    set rc [catch {
        set st [DboState]
        set lib [::capBridge::_findOpenLib $libPath $st]
        if { $lib eq "NULL" } {
            error "library is not open in this Capture session: '$libPath'; call capture_list_open_libraries first"
        }
...
proc ::capBridge::pkgInfo { pList } {
    ...
    set rc [catch {
        set st [DboState]
        set lib [::capBridge::_findOpenLib $libPath $st]
        if { $lib eq "NULL" } { error "library is not open: $libPath; call capture_list_open_libraries first" }
...
#
# Returns: {OK {schematic page refdes pinNumber x y} ...}
#############################################################################
            ...
            while { $pg != $NULL } {
                ...
                while { $inst != $NULL } {
                    ...
                    while { $pin != $NULL } {
                        ...
                        # The instance method GetHotSpot reports symbol-local
                        # coordinates on this 17.4 build (for example -10,30),
                        # which are useless for page wiring. The static
                        # DboPortInst accessors return the placed page point and
                        # are the same proven calls used by capPlaceWire.tcl.
                        set x ""; set y ""
                        catch {
                            set x [DboPortInst_sGetHotSpotX $pin $st]
                            set y [DboPortInst_sGetHotSpotY $pin $st]
                        }
                        lappend rows [list \
                            [::pcbWorkflows::_getDesignName $d] \
                            [::pcbWorkflows::_getPageLocation $pg] \
                            $rd $num $x $y]
                        set pin [$pit NextPin $st]
                    }
