...
from bridge.cadence_mcp import _page, _validate_arguments, build_tools  # noqa: E402
...
print("\nMCP schemas and pagination")
page_items, page_meta = _page(list(range(12)), {"offset": 5, "limit": 4})
check("page slice", page_items, [5, 6, 7, 8])
check("page has more", page_meta["has_more"], True)
check("page next offset", page_meta["next_offset"], 9)
check_raises(
    "page rejects excessive limit",
    lambda: _page([], {"limit": 501}),
    ValueError,
)
_schema = {
    "type": "object",
    "properties": {"name": {"type": "string"}, "limit": {"type": "integer", "maximum": 5}},
    "required": ["name"],
}
check_raises("schema requires field", lambda: _validate_arguments(_schema, {}), ValueError)
check_raises(
    "schema rejects unknown field",
    lambda: _validate_arguments(_schema, {"name": "x", "extra": 1}),
    ValueError,
)
check_raises(
    "schema enforces integer maximum",
    lambda: _validate_arguments(_schema, {"name": "x", "limit": 6}),
    ValueError,
)
_validate_arguments(_schema, {"name": "x", "limit": 5})
check("read-only MCP tool count", len(build_tools(False)), 22)
check("write-enabled MCP tool count", len(build_tools(True)), 26)
