...
**Both applications, one MCP server, 22 read-only tools and 4 opt-in write
tools.** Capture supports structured/paginated design inspection, engineering
audits, property modification, safe pin-to-pin wire preview/creation and save.
Allegro supports structured board reads, three packaged engineering audits and
arbitrary SKILL evaluation.

The MCP server is self-contained: it sources `pcbWorkflows.tcl`, the query
layer and the database wire constructor directly from this clone. No prior copy
into Cadence's global `tclscripts` directory is required.

### MCP tool coverage

| Area | Tools |
|---|---|
| Capture inventory | `capture_status`, `capture_list_pages`, `capture_list_parts`, `capture_list_pins`, `capture_list_nets`, `capture_connectivity` |
| Capture diagnosis | `capture_part_properties`, `capture_locate_net`, `capture_hanging_wires`, `capture_run_workflow`, `capture_engineering_audit` |
| Capture libraries | `capture_list_open_libraries`, `capture_list_library_packages`, `capture_get_package_info` |
| Capture mutation (`--allow-write`) | `capture_set_part_property`, `capture_connect_pins`, `capture_save_design` |
| Capture safe planning | `capture_preview_pin_connection` resolves pins/pages/absolute coordinates without writing |
| Allegro inventory | `allegro_status`, `allegro_drcs`, `allegro_symbols`, `allegro_nets` |
| Allegro engineering | `allegro_placement_audit`, `allegro_dangle_audit`, `allegro_silkscreen_audit` |
| Allegro advanced (`--allow-write`) | `allegro_eval` (arbitrary SKILL may mutate the board) |

Large Capture inventories are paginated (`offset`, `limit`; maximum 500).
Modern clients also receive `structuredContent`, output schemas and MCP tool
annotations alongside the human-readable report.

This enhanced distribution is based on
[`daveyJ-sgs/Cadence-MCP-SPB17`](https://github.com/daveyJ-sgs/Cadence-MCP-SPB17)
and preserves its MIT license and original attribution. The expanded MCP tool
surface, pagination, structured responses, safe write gating and regression
suite were added in this fork.

## Layout

| Path | Contents |
|---|---|
| `bridge/cadence_mcp.py` | **MCP server — 22 read-only + 4 opt-in write tools** |
...
| `bridge/test_bridge.py` | Offline protocol, schema and pagination regression tests (no Cadence tools needed) |
...
`capture_set_part_property`, `capture_connect_pins` and
`capture_save_design`. Use `capture_preview_pin_connection` before creating a
wire. Geometry created through the database layer can be saved; property edits
still require Capture's GUI save flow (see Gotchas below).
...
The test suite is dependency-free and does not require Cadence software:
...
`test_mcp_tools.py` invokes every published MCP tool against protocol-faithful
Capture and Allegro fakes, including the four opt-in write tools. See
[`TEST_REPORT.md`](TEST_REPORT.md) for the complete 26-tool matrix and the
separate live OrCAD Capture results. GitHub Actions repeats the offline suite
